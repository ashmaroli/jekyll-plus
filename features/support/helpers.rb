...
class Paths
  ...
  def self.output_file; test_dir.join("jekyll+_output.txt"); end

  def self.status_file; test_dir.join("jekyll+_status.txt"); end

  def self.jekyllplus_bin; source_dir.join("exe", "jekyll+"); end
...
def run_jekyll(args)
  ...
  process = run_in_shell("ruby", Paths.jekyllplus_bin.to_s, *args, "--trace")
